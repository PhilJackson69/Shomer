#!/usr/bin/env node
/**
 * Simple migration script to replace raw fetch() calls with apiFetch
...
function shouldExcludeFile(filePath) {
  return EXCLUDE_FILES.some(excludeFile => 
    filePath.includes(excludeFile)
  );
}

function shouldUseExternalFetch(filePath) {
  return EXTERNAL_FETCH_FILES.some(externalFile => 
    filePath.includes(externalFile)
  );
}
...
function migrateFile(filePath) {
  ...
  // Add appropriate import if not present and has fetch calls
  if (!hasApiFetchImport && !hasExternalFetchImport && content.includes('fetch(')) {
